...
try:
    ...
    if (
        "affiliation" in affiliation_subset.columns
        and affiliation_subset["affiliation"].notna().sum() > 0
    ):
        ...
        st.info(
            "No affiliation or contributor information is present for the selected collection."
        )
    ...
    if "general_modality" in df.columns and df["generalmodality"].notna().sum() > 0:
        ...
        wedges, _, _ = ax_mod.pie(
            modality_counts, labels=None, autopct="%1.1f%%", startangle=140
        )
        ax_mod.axis("equal")
        ax_mod.set_title("Dataset Count by Modality")
        ax_mod.legend(
            wedges,
            modality_counts.index,
            title="Modality",
            loc="center left",
            bbox_to_anchor=(1, 0.5),
            fontsize="small",
    ...
    if "frequencies" in df.columns and df["frequencies"].notna().sum() > 0:
        ...
        if freq_total:
            ...
            wedges, _, _ = ax6.pie(
                freq_series, labels=None, autopct="%1.1f%%", startangle=140
            )
            ax6.axis("equal")
    ...
    if "metadata" in df.columns:
        meta_counts = (
            df[df["collection"] == selected_collection]["metadata"]
            .value_counts()
            .to_dict()
        )
